"""The intervals printed in Table 2 and in Section 5.4, computed the way the paper describes them.

Both are two-stage cluster bootstraps: stage 1 resamples training seeds, stage 2 resamples the
calibration splits within each drawn seed. An earlier version of Table 2's per-group spread
column resampled seeds only, which leaves out the split-to-split component -- the larger of the
two (Table B7) -- and so printed intervals that were too narrow. This module is what the revised
numbers come from.

Model-level matching (Section 5.4) replaces ``accuracy_matching.py``, which pools every
(seed, split) point and is the procedure the revision withdraws. Here each training seed
contributes one (base accuracy, divergence) point, the mean over its calibration splits; ERM's
reference accuracy a* is its own mean; and the robust method's divergence at a* is read off a
least-squares line through its per-seed points.

    python -m study_robust_train.interval_tables
"""
...
RETAINED = ("kept", "flagged")          # the paper's retained runs: everything not excluded
...
def _two_stage_spread_ci(by_method: dict, *, B: int = DEFAULT_B, alpha: float = 0.05, seed: int = 0):
    """Two-sided CI for the across-method spread; same resampling as ``stats.spread_equivalence``."""
    prepared = []
...
def table2(records=None, *, B: int = DEFAULT_B):
    """Rows of Table 2 (calibration comparison, APS, rho_cal = rho_test = 0.95)."""
    ...
    for ds in DATASETS:
        for bb in BACKBONES:
            ...
            # the same ERM model under both rules, paired per (seed, split); ERM's values are
            # reported even where it is below its floor, which the table marks with a dagger
            pair = collections.defaultdict(dict)
            for r in rows:
                if r["method"] == "erm":
                    pair[(r["train_seed"], r["split_seed"])][r["calibration"]] = float(r["worst_group_cov"])
            ...
            out.append({
                ...
                "spread_shared": float(np.ptp([np.mean(v[0]) for v in spreads["marginal_split"].values()])),
...
def model_level_delta(records=None, *, dataset="waterbirds", backbone="resnet50_erm",
    ...
    """Delta(a*) = D_ERM(a*) - D_method(a*) at model level, with a two-stage cluster bootstrap."""
    ...
    def cloud(m):
        ...
        for r in recs:
            if (r["dataset"] == dataset and r["backbone"] == backbone and r["method"] == m
                    and r["score"] == score and r["rho_test"] == "0.95" and r["gate_status"] in RETAINED):
...
if __name__ == "__main__":
    ...
    print("Table 2")
    for row in table2():
        lo, hi = row["lift_ci"]; slo, shi = row["spread_per_group_ci"]
        print(f"  {row['dataset'][:2]}/{row['backbone']:14s} shared {row['shared']:.3f}  per-group "
              f"{row['per_group']:.3f}  lift {row['lift']:+.3f} [{lo:+.3f},{hi:+.3f}]  spread "
              f"{row['spread_shared']:.3f} / {row['spread_per_group']:.3f} [{slo:.3f},{shi:.3f}]  "
              f"one-sided upper {row['spread_per_group_upper_one_sided']:.4f}"
              + ("" if row["erm_retained"] else "  (ERM below floor)"))
    print("Section 5.4, GroupDRO-LL vs ERM on ResNet-50/Waterbirds")
    for sc in ("APS", "RAPS", "THR"):
        d = model_level_delta(score=sc)
        print(f"  {sc:4s} a*={d['a_star']:.4f}  Delta {d['delta']:+.3f} [{d['ci'][0]:+.3f},{d['ci'][1]:+.3f}]"
              f"  supports overlap: {d['supports_overlap']}")
