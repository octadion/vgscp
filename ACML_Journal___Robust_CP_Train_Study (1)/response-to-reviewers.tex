...
Passages set off by a blue rule are quoted from the revised manuscript. Because this letter compiles on its own, citations in those quotations are expanded to author and year and internal cross-references are dropped; nothing else is altered.
...
The revised manuscript runs to 45 pages against the 20 at submission: 22 of body and
declarations, 20 of appendices holding the requested tables, and 3 of references. We would rather
report the requested experiments and analyses in full than compress them, and we are glad to
...
\where{Section~5.1 (end), Section~5.5, and Appendix~C in full (Table~C12 and Figure~C1).}
...
\where{Section~4, new paragraph; Table~2 (intervals over seeds); Table~B10 (seed-versus-split variance); Appendix~F (seed sets).}
...
is $0.003$, so restoring them could not have moved the maximum of $0.024$ and does not; we say so
in the appendix rather than present a foregone number as evidence. Restoring the whole excluded
...
\where{Sections~5.3, 5.4 and 5.5 (moderation); Appendix~F with Table~F15 (reproducibility).}
...
within them: Tables~2, 4 and 6. Tables~3, 5 and 7 remain point estimates, and we say so rather than
claim more --- Table~3's across-seed spread is given separately in Appendix~B, Table~5 reports
...
twenty-two tables now pass a column-count check, none has a column that is blank in every row, and
...
reported as failing in seven of eight settings. We believe the paper's central finding is stronger for
it, since it now rests on eight backbone--dataset settings and on a study in which the representation
