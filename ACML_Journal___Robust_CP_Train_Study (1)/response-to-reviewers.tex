...
representation itself is retrained. Every table in the paper comes from these.
...
$0.8695$ on Waterbirds and $0.8847$ on CelebA; we observe $0.8660$ and $0.8800$, with $98\%$ and
$95\%$ of runs inside the simulated central range.
\end{revised}
...
we state the three protocol differences that explain it: our runs refit only the last layer on
...
accuracy is read on our pooled evaluation domain rather than the benchmark test split. DFR, the one method whose protocol matches its published counterpart, is correspondingly the closest ($0.851$
...
\where{Section~4, new paragraph; Table~2 (intervals over seeds); Table~B7 (seed-versus-split variance); Appendix~F (seed sets).}
...
(across-seed SD $0.003$--$0.027$ in worst-group accuracy) and none for ERM or AFR. This is also why
every interval here is a cluster bootstrap that resamples training seeds and then calibration
...
applied run by run to whichever group is in fact worst. The assumption is a good approximation for
the ERM baseline under a shared threshold, where one group is the worst in $85$--$100\%$ of runs,
and a poor one under per-group thresholds, where no group holds the minimum in more than $41\%$ of
runs and the most concentrated setting is closer to the $25\%$ of a four-way tie than to the
...
all eight settings, and under per-group thresholds it is near a four-way tie. Nothing in the paper
needed the assumption --- the identity and the bound hold for whichever group is worst in a given
run --- so the claim has been withdrawn rather than repaired, and Appendix~A now reports where the
...
compositing to $\rhocal=0.95$ leaves that group holding $47.5\%$, so the shortfall there is not the
...
different questions. The three individual seeds all sit in one setting whose across-method spread
is $0.003$, so restoring them could not have moved the maximum of $0.024$, and does not. The
...
GroupDRO-LL's. We say so rather than offer AUROC
...
\textbf{uninformative in five of eight settings} once a cluster-bootstrap CI is attached --- each rests
...
\where{Title, Section~5.1 heading and fifteen further places (narrowing); Table~2 and all
main tables (CIs); Table~2 and Section~5.1 (equivalence test on coverage); Section~5.3 (equivalence
...
a Calibration Property, Not a Representation One'' to ``The Calibration Rule Moves Coverage
More Than Training Does''.

\textbf{Confidence intervals.} The estimates that carry the paper's claims now come with a
two-stage cluster bootstrap interval that resamples training seeds and then calibration splits
within them: Tables~2, 4 and 6. Tables~3, 5 and 7 remain point estimates and are labelled as such --- Table~3's across-seed spread is given separately in Appendix~B, Table~5 reports
...
comment is about: a two-one-sided-test check of whether the across-method spread under per-group
thresholds is equivalent to zero within a $0.05$ margin, which we chose after seeing the
spreads and state as post-hoc in the table caption. It is, in all eight settings --- the
training method stops mattering once calibration is group-conditional. On \emph{set size}, the
same check against a $0.10$ margin \textbf{rejects} equivalence in six of eight settings, which is
why we withdrew the description of the efficiency range as narrow.
...
\where{Abstract, introduction, Section~5.3 and the discussion (conditioning); Table~4
(intervals).}
...
of a deterministic relationship. Table~4 now prints each interval, italicised where it spans
essentially $[-1, +1]$, and the text reports a direction rather than an estimate.

\section*{Further corrections}

Three defects we found ourselves while revising. No review raised them, and we list them here so
that none is a silent change.

\begin{enumerate}[leftmargin=1.4em,itemsep=0.35em]
\item \textbf{A definitional inconsistency in Section~3.} The coverage gap was defined in a way
that contradicted both the implementation and the submitted paper's own appendix. It now reads
...
supplied seven, leaving a stray empty column. Fixed during the rewrite; all
eighteen tables now pass a column-count check, none has a column that is blank in every row, and
each was measured against the text block so none runs into the margin.
\item \textbf{An empty-set disclosure.} On DINOv2/Waterbirds the mean worst-group set size is
\emph{below one} ($0.888$--$0.962$ across methods and scores), meaning a small fraction
of prediction sets are empty. This is a legitimate outcome of standard split conformal with a highly accurate model, and
